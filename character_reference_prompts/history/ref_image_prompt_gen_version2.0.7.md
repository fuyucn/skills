...
# ref_image_prompt_gen_version2.0.7.md
...
# Expression Freedom（2.0.7 新增）

## 问题

身份锁定语言过强时，模型会把参考图的表情一起锁死：新图脸是像的，但笑容僵硬、像冻住一样，缺少自然生命力。

## 核心规则

```text
身份 = 静态结构：脸型、骨相、五官形状、皮肤质感、年龄感、痣的位置。
表情 = 瞬间状态：笑容、嘴形、眼睛睁合、眉毛状态、头部角度、视线方向。
```

1. 静态结构必须与参考图一致；表情、视线、头部角度必须由新场景重新生成，允许并且应该和参考图不同。
2. 用户没有指定表情时，不写 `expression consistent with the reference image`，改用场景自然表情。
3. 用户指定表情时（笑、张嘴笑、安静看镜头等），直接写这个表情，并强调脸部肌肉放松自然、不僵硬。
...
```text
Same face, new expression: the facial structure stays identical to the reference, but the expression belongs to this new moment — relaxed, natural, alive, never copied or frozen from the reference photo.
```

Avoid 表情段：

```text
Do not copy the reference photo's expression, head angle, or gaze. Do not freeze the face or stiffen the smile. Keep facial muscles relaxed and natural.
```
...
```text
natural smile / soft smile / cheerful open-mouth laugh / neutral expression / calm look
```
...
规则：最多 1 个表情状态，不写微表情和心理戏。用户无要求时写场景自然表情（如 `a relaxed natural expression fitting the scene`），不要写 `expression consistent with the reference image`。表情由当前场景重新生成，可以与参考图不同；同一张脸可以笑、可以不笑，但骨相必须一致。
...
```text
Do not copy the reference photo's expression, head angle, or gaze. Do not freeze the face or stiffen the expression. Keep facial muscles relaxed and natural.
```
...
```
Repair: Same face, new expression. The expression belongs to this scene, not the reference photo — relaxed, natural, alive. Do not freeze the face, do not stiffen the smile, do not copy the reference expression.
```
...
The person in this image is the same individual as the uploaded reference photo — their facial identity, bone structure, and features are identical. Generate this same person in a new scene with new styling. The reference image defines who they are; the text below only describes where they are, what they wear, and how they pose. Same face, new expression: the expression, gaze, and head angle belong to this new scene, not the reference photo.
...
Use one natural pose: [pose/action]. Same face, new expression: give the face a relaxed, natural, alive expression that belongs to this new scene — do not copy or freeze the reference photo's expression. Keep the face clear and recognizable from the reference image.
...
Reference image #1 is the identity source for Character 1. Reference image #2 is the identity source for Character 2. Each person in this image is the same individual as their respective reference photo — their facial identities, bone structures, and features are identical. Generate these same two people in a new scene. Do not swap faces between characters. Each person's expression, gaze, and head angle are created fresh for this new scene — do not copy or freeze expressions from either reference. Both faces should be naturally illuminated and shaded with consistent light direction, color temperature, and shadow density matching the rest of their bodies.
...
Do not copy either reference photo's expression, head angle, or gaze. Do not freeze either face. Keep both faces relaxed and natural.
...
6. 动作简洁，不写复杂心理戏。
7. 表情极简，不写微表情和气质故事，但必须是新场景生成的放松自然表情，不得复制或冻结参考图表情。
