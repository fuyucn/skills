...
description: |
  ...
  当前规范为 ref_image_prompt_gen 2.0.7：身份锁定 + 表情解冻、开头结尾框架、场景光融合、hypn 签名注入、失败修复。
---

# 人物参考图模式提示词

本 Skill 是 `ref_image_prompt_gen` 的完整规范库。**当前唯一现行版本是 2.0.7**，生成提示词时直接读取：

→ [ref_image_prompt_gen_version2.0.7.md](./ref_image_prompt_gen_version2.0.7.md)
...
- 身份锁定只锁静态面部结构；表情、视线、头部角度由新场景重新生成，不复制参考图表情。
...
4. 使用开头声明 + 结尾排除框架（Option C），或按 2.0.7 的身份陈述选项。
