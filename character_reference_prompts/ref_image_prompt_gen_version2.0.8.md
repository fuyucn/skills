...
version: 2.0.8
description: |
  参考图人像生成提示词优化 skill。代码块只输出 Main Prompt + Avoid，去除元信息。
  支持单人/双人/多人参考图模式。核心原则：参考图决定"是谁"；文本只决定"在哪里、穿什么、做什么、怎么拍"。
  2.0.6 新增：开头+结尾框架（Option C）、场景光融合升级、Failure Repair 速查。
  2.0.7 新增：表情解冻——身份只锁静态面部结构，表情/视线/头部角度由新场景重新生成。
  2.0.8 新增：表情克制——表情只允许同一张脸的肌肉张力变化，禁止重塑五官、夸张表情导致崩坏。
  包含面部融合口诀 + 眼神有神规则。
  自动注入艺术签名 "hypn"。
---

# ref_image_prompt_gen_version2.0.8.md
...
9. **（2.0.5 新增）** 提示词必须引导模型将人物视为"同一人在新场景中重新生成"，而非"将参考图的脸贴到新身体上"。通过身份语气（Identity Phrasing）和光影融合描述杜绝拼贴感。
10. **（2.0.6 新增）** 提供 Option C 开头+结尾框架——开头声明"同一个人"，结尾明确排除"不要复制背景、服装、表情"，消除模型歧义。
11. **（2.0.6 新增）** 场景光融合使用画面描述式语言（`one continuous light binding to the scene`）替代检查列表式语言。
12. **（2.0.6 新增）** 提供 Failure Repair 速查——每类故障一句可复制修复词。
13. **（2.0.7 新增）** 身份锁定只针对静态面部结构（骨相、五官、脸型、皮肤质感、年龄感、痣）；表情、视线、头部角度属于新场景的瞬间状态，必须重新生成，不得复制参考图表情。
14. **（2.0.8 新增）** 表情必须克制：只允许同一张脸上的肌肉张力变化，禁止重塑、拉伸、夸张表情导致五官崩坏。
...
# Face Integration & Re-Identity（2.0.5 新增）

## 核心问题

有时生成结果会让人脸看起来像"贴上去的"——面部与身体脱节、光照方向不一致、皮肤质感不统一。这是提示词的身份陈述方式错误造成的。
...
不要把人物当作"两张图的拼接"：
  ❌ "把参考图A的脸放到场景B的身体上"
  ✅ "这个人是参考图中的同一个人，在新的场景中重新生成"
```

**模型应该理解的是：** 参考图只是告诉你这个人长什么样，而你要做的是在新场景中**重新生成**这个完整的人，而不是把脸剪切粘贴过去。

## 身份陈述语气（Identity Phrasing）对照表

| 错误语气（拼贴感） | 正确语气（融合感） |
|---|---|
| Use the reference image as face source. Copy this face onto the scene. | This person is the same individual as the reference photo — their facial identity, bone structure, and features are identical. Generate them in a new scene. |
| Replace the face of the model with the reference face. | The reference image defines who they are; the text below only describes where they are, what they wear, and how they pose. |
| Put this face on this body. | Generate this same person in a new scene with new styling. |

## 光影融合规则

即使身份陈述正确，如果面部与身体的光线方向、色温、对比度不一致，仍然会产生拼贴感。

规则：

1. **受光方向一致**：面部主光方向必须与身体/场景的主光方向相同。
   - 如果场景光源来自左上，面部高光也必须在左上。
   - 避免面部平光 + 身体侧光的矛盾。
2. **色温一致**：面部的色温（暖/冷/中性）必须与身体皮肤一致。
   - 如果场景是暖色钨丝灯，面部不应是冷白日光色。
3. **阴影密度一致**：面部的阴影硬度和密度必须与场景匹配。
   - 硬光场景（直闪/阳光）→ 面部有清晰阴影边界。
   - 柔光场景（窗光/柔光箱）→ 面部阴影柔和过渡。
4. **皮肤质感一致**：面部的纹理粗细、光泽度必须与身体一致。
   - 如果身体有汗水/油光/水珠，面部也应有对应质感。
   - 避免面部磨皮 + 身体写实纹理的矛盾。

## 身份陈述推荐开头

在 Main Prompt 开头使用以下语气之一：

选项 A（自然融合式）：

```text
The person in this image is the same individual as the uploaded reference photo — their facial identity, bone structure, and features are identical. Generate this same person in a new scene with new styling. The reference image defines who they are; the text below only describes where they are, what they wear, and how they pose. The person's face should be naturally illuminated and shaded in the same way as the rest of their body, with consistent light direction, color temperature, and shadow density. No compositing or cut-and-paste appearance.
```
...
**默认推荐使用选项 A**。只有在选项 A 导致身份识别失败时才回退到选项 B。

选项 C（开头+结尾框架，2.0.6 新增）：

吸收 ref-to-image 项目的已验证经验。核心逻辑：模型对"同一个人"的理解有歧义——它可能理解为"复制参考图全部内容 + 换文字描述的场景"，结果就是 PS 贴脸。必须在开头声明身份，在结尾明确排除不要复制的元素。

开头：

```text
这是一张参考照片。请生成一张全新的照片，照片中的人物必须是参考图里的**同一个人**，绝对不能变成另一个人。
```

结尾（必须放在 Main Prompt 最后）：

```text
【最后重申】
再次强调：这必须是参考图里的同一个人，骨相、五官、脸型、痣的位置都必须和参考图完全一致。不要复制参考图的背景、服装、表情、头部角度和视线；表情必须由当前场景重新生成，自然放松、不僵硬。生成一张全新的、自然的照片。
```

使用场景：当选项 A 反复出现 PS 贴脸感时，切换到选项 C。选项 C 的头尾框架可与选项 A 的开头语句组合使用。

---

# Expression Freedom & Stability（2.0.7 新增，2.0.8 收紧）

## 问题

2.0.6 的问题是表情被锁死；2.0.7 放开表情后，模型为了生成大笑等动态表情会重新排布面部肌肉，导致嘴、眼、下巴结构崩坏。表情必须自由，但变化幅度必须被限制在"同一张脸的肌肉张力变化"范围内。

## 核心规则

```text
身份 = 静态结构：脸型、骨相、五官形状、皮肤质感、年龄感、痣的位置。
表情 = 肌肉张力变化：嘴角、眼睛、眉毛、下颌在同一张脸上自然运动。
```

1. 静态结构必须与参考图一致；表情只能改变肌肉张力，不能改变五官形状、比例、位置或脸型轮廓。
2. 用户没有指定表情时，不写 `expression consistent with the reference image`，改用简单自然表情（如 `a soft natural smile`）。
3. 用户指定大笑、张嘴笑等强表情时，必须改写为轻、自然、克制的版本：`mouth naturally open in a light laugh, not extreme`。表情幅度越大，崩坏风险越高。
4. 表情变化越小越安全：`soft smile / gentle laugh / calm look` 优先；`exaggerated laugh / big grin / extreme mouth opening` 禁止。
...
```text
Same face, same facial structure: the face is never reshaped, stretched, or redrawn. Only natural muscle tension changes for the expression — a small, relaxed, natural range. Keep the expression subtle and calm; if the scene calls for a laugh, keep it light and natural, never extreme, never exaggerated.
```

用户明确要大笑容时的改写：

```text
Same face, same facial structure. The expression is a cheerful light laugh with the mouth naturally open and the jaw relaxed — small natural range, no extreme opening, no reshaped features.
```

Avoid 表情段：

```text
Do not copy the reference photo's expression, head angle, or gaze. Do not freeze the face or stiffen the smile, but do not exaggerate the expression either. No reshaped, stretched, or distorted facial features. No deformed mouth, no distorted smile, no misshapen eyes, no stretched jaw, no extra teeth, no exaggerated facial muscles. Keep expression changes within a small natural range.
```

---

# Scene Light Fusion（2.0.6 升级）

## 从检查列表到画面描述

旧版（2.0.5）的光融合描述是检查列表式：

```text
with consistent light direction, color temperature, and shadow density
```

这是给人类质检用的语言，不是模型能视觉化理解的画面描述。2.0.6 升级为画面描述式：

## 基础融合句（全部场景适用）

```text
One continuous light binds the subject to the scene — the light touches their face the same way it touches the background. Not separately lit, not a face inserted from another source, but one person in one continuous photograph.
```
...
| 光线类型 | 融合句 |
|---|---|
| 窗光/室内自然光 | The shadow on their face exists because the window casts it — one continuous light, one moment, one photograph. |
| 夜景街灯 | The warm streetlamp light that falls across the pavement also wraps around their face — one single light source, no separate face lighting. |
| 直闪/棚灯 | The flash illuminates face and body as one — no dark halo around the jaw, no face lit from a different angle than the shoulders. |
| 室外自然光 | The same sunlight that catches their hair also traces their cheekbone and shoulder — a single continuous outdoor light, no composite feel. |
...
```text
natural smile / soft smile / gentle laugh with mouth lightly open / neutral expression / calm look
```
...
defiant gaze / lips slightly parted / innocent look / extreme open-mouth laugh / big grin / exaggerated laugh
```

规则：最多 1 个表情状态，不写微表情和心理戏。表情优先选轻微、自然、克制的版本：`soft smile / gentle laugh / calm look`。用户要求大笑或张嘴笑时，转写为 `a light cheerful laugh with the mouth naturally open, jaw relaxed`，并强调不夸张、不重塑五官。用户无要求时写 `a soft natural smile fitting the scene`，不要写 `expression consistent with the reference image`。

## Eyes & Gaze Rules（2.0.5 新增）

眼神是画面灵魂。`steady/direct/composed` 不等于呆板空洞。

1. **眼神必须有神**：描述眼神时不能只用"steady/direct/composed"三个静态词。必须加入至少一个体现"有生命感"的修饰。
   - ✅ `steady but not rigid — a quiet presence behind the eyes`
   - ✅ `calm and focused, with a faint trace of thought`
   - ✅ `still but alive — not blank, not staring`
   - ❌ `steady, direct, composed`
2. **不写微表情**：反对的是过度心理戏（"她的眼神诉说着千言万语"），不是反对眼神的生命力。
3. **方向区分**：
   - 直视镜头：`looking into the lens with presence, not staring through it`
   - 看向一侧：`gaze soft, directed slightly past the camera`
   - 闭眼：`eyes gently closed, lids relaxed, not pressed tight`
4. **避免的坑**：
   - `blank stare` / `empty look` / `dead eyes` / `glazed over` / `staring vacantly`
   - 这些词本身会引导模型生成空洞的眼神
   - 如果需要"放空"感，用 `gaze soft and slightly unfocused, as if thoughts are elsewhere`
...
```text
Avoid distorted face, blurry face, face too small, bad eyes, bad hands, deformed hands, extra fingers, missing fingers, broken anatomy, distorted body, and stiff frozen expression.
```

## Image Quality

```text
Avoid low resolution, heavy blur, noise, artifacts, watermark, logo, caption, username, signature, random text, microphone, mic, visible recording device.
```
...
## Integration Check（2.0.5 新增）

确认以下内容：

1. **身份陈述语气正确**：Main Prompt 开头使用的是"同一人在新场景"语气（选项 A），而不是"贴脸"语气。
2. **面部受光方向一致**：光线描述中，面部与身体的受光方向、色温、阴影密度描述一致，没有矛盾。
3. **无拼贴暗示**：没有使用 copy / paste / transfer / replace face 等拼贴暗示词。
4. **皮肤质感统一**：面部与身体的皮肤质感描述一致（都写 real 或都写 soft，不会一个磨皮一个写实）。
...
## 如果面部像贴上去的 / 拼贴感（2.0.5 新增）

检查：

1. Main Prompt 开头是否使用了"贴脸"语气（"copy face onto..."、"replace face with..."）。
2. 面部与身体的光线方向是否一致。
3. 面部与身体的色温是否一致。
4. 面部与身体的皮肤质感描述是否矛盾（面部磨皮 + 身体写实 = 拼贴感）。
5. 是否缺少光影融合描述。
...
改用"同一人在新场景"语气（选项 A）。
在光线描述中加入：
  The person's face should be naturally illuminated and shaded in the same way as the rest of their body, with consistent light direction, color temperature, and shadow density. No compositing or cut-and-paste appearance.
确保面部与身体的肤感描述一致。
```

---

# Failure Repair Quick Reference（2.0.6 新增）

不提供诊断逻辑（前面的 Troubleshooting 已经覆盖），直接提供**可复制使用的一句修复词**。找到对应问题，将该句替换到 Main Prompt 的光线段或身份段。

## 不像本人

```
Repair: Exact same face as reference. Target facial similarity: 95-100%.
Preserve same eyes, nose, lips, jawline, cheekbones, forehead, face shape, skin tone, age impression.
```

将此段放在 Main Prompt 结尾处，签名之前。

## PS 贴脸 / 脸冻住

```
Repair: Same face as reference. The light in this scene falls across the face the same way it fills the room — one continuous light. Not a frozen copy. Not a pasted face.
```

将此段替换 Main Prompt 的光线段。

## 面部与身体光影不一致

```
One continuous light binds the subject to the scene — the light touches their face the same way it touches the background. Not separately lit, not a face inserted from another source. No dark halo around the jaw, no differently lit face.
```

## 过度美化

```
Repair: Same face. Do not beautify. Do not make eyes larger, nose bridge higher, chin sharper, skin whiter. Preserve real skin texture, natural asymmetry.
```

## 表情僵硬 / 复制参考图表情

```
Repair: Same face, same facial structure. The expression belongs to this scene, not the reference photo — relaxed, natural, small range. Do not freeze the face, do not stiffen the smile, do not copy the reference expression.
```

将此段替换 Main Prompt 的表情段；同时在 Avoid 中加入 `Do not copy the reference photo's expression, head angle, or gaze.`

## 表情崩坏 / 五官变形

```
Repair: Same face, same facial structure — never reshaped, stretched, or redrawn. Reduce the expression to a small natural range: soft smile, relaxed jaw, natural eyes. No deformed mouth, no distorted smile, no misshapen eyes, no extra teeth.
```

将此段替换 Main Prompt 的表情段，并把表情动作改为 light / gentle 版本。

## 眼神呆板空洞

```
Repair: Eyes alive and present — not blank, not staring, not dead. A quiet presence behind the gaze.
```
...
The person in this image is the same individual as the uploaded reference photo — their facial identity, bone structure, and features are identical. Generate this same person in a new scene with new styling. The reference image defines who they are; the text below only describes where they are, what they wear, and how they pose. Same face, same facial structure: expression changes only as natural muscle tension on the same face — subtle, relaxed, never reshaped or exaggerated.
...
Use one natural pose: [pose/action]. Same face, same facial structure: keep the expression in a small natural range — a soft smile or light laugh, relaxed and alive, not copied from the reference and not exaggerated. Keep the face clear and recognizable from the reference image.

[Lighting, camera, composition, color, texture, mood, style.]

One continuous light binds the subject to the scene — the light touches their face the same way it touches the [background element]. Not separately lit, not a face inserted from another source, but one person in one continuous photograph.
...
Do not copy the reference photo's expression, head angle, or gaze. Do not freeze the face or stiffen the smile, but do not exaggerate the expression either. No reshaped, stretched, or distorted facial features. No deformed mouth, no distorted smile, no misshapen eyes, no stretched jaw, no extra teeth. Keep expression changes within a small natural range.
...
Avoid face obstruction, hair covering the face, hands covering the face, props covering the face, low resolution, heavy blur, noise, artifacts, watermark, logo, caption, username, signature, random text, microphone, mic, visible recording device.
...
Reference image #1 is the identity source for Character 1. Reference image #2 is the identity source for Character 2. Each person in this image is the same individual as their respective reference photo — their facial identities, bone structures, and features are identical. Generate these same two people in a new scene. Do not swap faces between characters. Each person's expression stays within a small natural range — subtle muscle tension on the same face, never reshaped or exaggerated. Both faces should be naturally illuminated and shaded with consistent light direction, color temperature, and shadow density matching the rest of their bodies.
...
Do not copy either reference photo's expression, head angle, or gaze. Do not freeze either face, but do not exaggerate either expression. No reshaped, stretched, or distorted features on either face. Keep both expressions within a small natural range.
...
Avoid low resolution, heavy blur, noise, artifacts, watermark, logo, caption, username, random text, microphone, mic, visible recording device.
...
6. 动作简洁，不写复杂心理戏。
7. 表情极简、幅度克制，不写微表情和气质故事；表情必须是同一张脸上小幅、自然的肌肉张力变化，不得复制参考图表情，也不得夸张导致五官变形。
...
14. 身份陈述使用"同一人在新场景"语气（选项 A），而非"贴脸"语气。
15. 面部与身体的受光方向、色温、阴影密度、皮肤质感描述一致，无拼贴感。
16. **（2.0.6 新增）** 光线使用画面描述式语言（`one continuous light binds...`），而非检查列表式语言（`consistent light direction, color temperature...`）。
17. **（2.0.6 新增）** Main Prompt 包含【最后重申】段落，明确排除"不要复制背景、服装和表情"。
18. **（2.0.7 新增）** 身份锁定只覆盖静态面部结构；表情、视线、头部角度由场景重新生成，Avoid 中明确禁止复制参考图表情。
19. **（2.0.8 新增）** 表情变化只允许同一张脸的肌肉张力变化；禁止重塑、拉伸或夸张表情导致五官崩坏。
